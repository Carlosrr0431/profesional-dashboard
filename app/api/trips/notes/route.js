...
import { updateTripNotesAsOperator } from '../../../../src/lib/updateTripNotesAsOperator';
...
export async function PATCH(request) {
  ...
  if (!tripId) {
    return NextResponse.json(
      { ok: false, message: 'Falta el viaje.' },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const result = await updateTripNotesAsOperator(supabase, tripId, body?.notes ?? '');
    return NextResponse.json({
      ok: true,
      trip: result.trip,
    ...
    return NextResponse.json(
      { ok: false, message: err?.message || 'No se pudieron enviar las notas.' },
