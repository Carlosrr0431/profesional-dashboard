...
const MAX_AUTOCOMPLETE_VARIANTS = 6;
...
export const reverseGeocode = async (lat, lng) => {
  ...
  try {
    ...
    return formatShortAddress(label) || label || fallback;
  } catch (error) {
...
/**
 * Búsqueda de direcciones en Salta Capital vía Nominatim + salta-address.
 */
export const autocompleteAddressSalta = async (query, limit = 8) => {
  ...
  try {
    ...
    for (const variant of searchQueries) {
      const results = await searchNominatimVariant(variant, Math.max(limit, 8));
      for (const item of results) {
        const placeId = item.placeId;
        const address = item.formattedAddress;
        const addressKey = String(address || '').toLowerCase().trim();
        ...
        merged.push({
          ...
          lat: item.lat,
          lng: item.lng,
          score: scoreCandidateAgainstQuery(address, trimmed),
        });
      }
    }
    ...
    return merged.slice(0, limit).map(({ address, placeId, lat, lng }) => {
      const { title, subtitle } = formatAddressSuggestion(address);
      return {
        address,
        placeId,
        lat,
