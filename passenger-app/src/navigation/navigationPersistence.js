...
const STORAGE_KEY = '@passenger_app/navigation_state_v1';

const runtime = globalThis.__passengerAppNavigationPersistenceRuntime ??= {
...
export function flushNavigationPersistence() {
  const { hasProfile, isLoading } = useAuthStore.getState();
  if (!hasProfile || isLoading || !navigationRef.isReady()) return;
...
function installAppStateFlushListener() {
  const flag = '__passengerAppNavigationFlushListener';
