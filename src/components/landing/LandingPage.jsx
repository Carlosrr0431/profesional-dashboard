    ...
    >
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-white via-light-50 to-light-100">
        ...
        <div className="relative mx-auto max-w-6xl px-4 pb-20 pt-8 sm:px-6 sm:pb-24 sm:pt-12 lg:px-8 lg:pt-16">
          ...
          <h1 className="landing-hero-enter landing-hero-enter-delay-1 max-w-4xl text-[2rem] font-extrabold leading-[1.08] tracking-tight text-navy-900 sm:text-5xl lg:text-6xl">
            Tu viaje en Salta,{' '}
            <span className="landing-shimmer-text">más simple</span>
            {' '}que nunca
          </h1>

          <p className="landing-hero-enter landing-hero-enter-delay-2 mt-5 max-w-2xl text-base leading-relaxed text-slate-600 sm:mt-6 sm:text-lg lg:text-xl">
          ...
          <div className="landing-hero-enter landing-hero-enter-delay-4 mt-10 grid grid-cols-2 gap-2.5 sm:mt-14 sm:grid-cols-4 sm:gap-4">
            {STATS.map((stat) => (
              <div
                key={stat.label}
                className="rounded-2xl border border-light-300/80 bg-white/80 px-3 py-3 shadow-sm backdrop-blur-sm sm:px-4 sm:py-4"
              >
                <p className="text-lg font-bold text-navy-900 sm:text-2xl">{stat.value}</p>
      ...
      {/* Intro apps */}
      <section id="apps" className="border-y border-light-300/70 bg-white py-12 sm:py-16">
        <div className="mx-auto max-w-6xl px-4 text-center sm:px-6 lg:px-8">
          <RevealSection>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-accent sm:text-sm">Dos apps, una plataforma</p>
            <h2 className="mt-3 text-2xl font-extrabold tracking-tight text-navy-900 sm:text-3xl lg:text-4xl">
      ...
      {/* Features */}
      <section id="features" className="border-t border-light-300/70 bg-white py-16 sm:py-24 lg:py-28">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <RevealSection className="mx-auto mb-10 max-w-2xl text-center sm:mb-14">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500 sm:text-sm">Tecnología</p>
            <h2 className="mt-3 text-2xl font-extrabold tracking-tight text-navy-900 sm:text-3xl lg:text-4xl">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {PLATFORM_FEATURES.map((feature, index) => (
              <RevealSection key={feature.title} delay={index * 80}>
                <div className="group h-full rounded-2xl border border-light-300/80 bg-light-100/50 p-6 transition duration-300 hover:border-navy-700/15 hover:bg-white hover:shadow-[0_16px_40px_-12px_rgba(15,23,42,0.1)]">
                  <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-navy-700 shadow-sm transition group-hover:bg-accent/10 group-hover:text-accent">
                    {feature.icon}
                  </span>
      ...
      {/* How it works */}
      <section id="how" className="border-t border-light-300/70 bg-light-100/80 py-16 sm:py-24 lg:py-28">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <RevealSection className="mx-auto mb-10 max-w-2xl text-center sm:mb-14">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500 sm:text-sm">Cómo funciona</p>
            <h2 className="mt-3 text-2xl font-extrabold tracking-tight text-navy-900 sm:text-3xl lg:text-4xl">
          ...
          <div className="grid gap-5 sm:gap-8 lg:grid-cols-2">
            {[
              ...
              <RevealSection key={block.title} delay={blockIndex * 100}>
                <div className="h-full rounded-[1.5rem] border border-light-300/80 bg-white p-6 shadow-sm sm:rounded-[2rem] sm:p-8">
                  <h3 className="text-lg font-bold text-navy-900 sm:text-xl">{block.title}</h3>
      ...
      {/* CTA operadores */}
      <section className="border-t border-light-300/70 bg-white py-16 sm:py-24">
        <RevealSection>
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div className="relative overflow-hidden rounded-[1.75rem] border border-navy-700/10 bg-gradient-to-br from-navy-900 via-navy-700 to-[#245f8d] p-6 shadow-[0_24px_60px_-20px_rgba(15,23,42,0.35)] sm:rounded-[2rem] sm:p-10 lg:p-12">
          ...
          <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 sm:gap-8">
            <div>
              ...
              <ul className="mt-3 space-y-2 text-sm text-slate-600">
                <li><a href={PLAY_PASSENGER} target="_blank" rel="noopener noreferrer" className="transition hover:text-navy-900">Pasajero</a></li>
                <li><a href={PLAY_DRIVER} target="_blank" rel="noopener noreferrer" className="transition hover:text-navy-900">Conductor</a></li>
              </ul>
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-slate-500">Legal</p>
              <ul className="mt-3 space-y-2 text-sm text-slate-600">
                <li><Link href="/privacidad" className="transition hover:text-navy-900">Privacidad</Link></li>
                <li><Link href="/eliminacion-cuenta" className="transition hover:text-navy-900">Eliminar cuenta</Link></li>
              </ul>
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-slate-500">Soporte</p>
              <ul className="mt-3 space-y-2 text-sm text-slate-600">
                <li><Link href="/contacto" className="transition hover:text-navy-900">Contacto</Link></li>
                <li><Link href="/admin/login" className="transition hover:text-navy-900">Operadores</Link></li>
              </ul>
            </div>
          </div>
        </div>

        <div className="mx-auto mt-8 max-w-6xl border-t border-light-300/70 px-4 pt-6 sm:mt-10 sm:px-6 sm:pt-8 lg:px-8">
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-0 z-40 border-t border-light-300/80 bg-white/95 p-3 backdrop-blur-xl sm:hidden pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        <div className="pointer-events-auto mx-auto flex max-w-lg gap-2">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex flex-1 items-center justify-center rounded-xl border border-light-300 bg-light-100 px-3 py-3 text-xs font-bold text-navy-900"
          >
            App Conductor
          </a>
        </div>
