...
const STALE_PUSH_TOKEN_REASONS = new Set(['device_not_registered', 'invalid_registration_token']);

/**
 * Send push notification through the backend FCM endpoint.
 */
export async function sendPushNotification(pushToken, { title, body, data = {}, driverId = null } = {}) {
  if (!pushToken) {
  ...
  try {
    ...
    if (!response.ok || !result?.ok) {
      const reason = result?.reason || String(response.status);
      if (STALE_PUSH_TOKEN_REASONS.has(reason)) {
        if (result?.tokenCleared) {
          console.warn('El dispositivo del chofer ya no está registrado; se limpió el push token.');
        } else {
          console.warn('El dispositivo del chofer ya no está registrado; no se pudo enviar la notificación push.');
        }
      } else {
        console.error('Push notification error:', reason);
      }
    }
    return result;
  } catch (error) {
