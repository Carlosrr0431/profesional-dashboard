            ...
            <div className="flex flex-col gap-2">
              {[
                ...
                },
                {
                  ...
                  hint: 'Siempre puede recibir viajes. Solo se bloquea manualmente desde el panel.',
                },
              ].map((opt) => (
                <button
                  key={opt.value}
                  ...
                  className={`text-left rounded-xl border px-3 py-2.5 transition-all ${
                    form.billing_mode === opt.value
                      ? 'bg-accent/10 border-accent/30'
                      : 'bg-light-200 border-light-300/50 hover:border-light-400'
                  }`}
                >
                  <p className={`text-sm font-semibold ${form.billing_mode === opt.value ? 'text-accent' : 'text-navy-900'}`}>
                    {BILLING_MODE_LABELS[opt.value]}
                  </p>
