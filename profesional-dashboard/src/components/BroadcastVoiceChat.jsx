...
export default function BroadcastVoiceChat({ drivers, onClose }) {
  ...
  const getSupportedMime = () => {
    if (typeof MediaRecorder !== 'undefined') {
      // Priorizar MP4/AAC — compatible con Android y iOS
      if (MediaRecorder.isTypeSupported('audio/mp4;codecs=mp4a.40.2')) return { mimeType: 'audio/mp4;codecs=mp4a.40.2', ext: 'm4a', contentType: 'audio/mp4' };
      if (MediaRecorder.isTypeSupported('audio/mp4')) return { mimeType: 'audio/mp4', ext: 'm4a', contentType: 'audio/mp4' };
  ...
  const handleSend = async () => {
    ...
    if (!rawBlob || rawBlob.size === 0 || drivers.length === 0) return;

    setSending(true);
    try {
      const wavBlob = await encodeToWav(rawBlob);
      const timestamp = Date.now();
      const fileName = `broadcast/base-${timestamp}.wav`;
